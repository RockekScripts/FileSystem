...
static int my_unlink(const char * path){
	 ...
	 if(!resizeNode(idxDir,0))
		 return -1;

	 myFileSystem.nodes[idxDir]->freeNode = true;
	 myFileSystem.numFreeNodes++;
	 myFileSystem.directory.numFiles--;
...
static int my_read(const char * pat,char *buff, size_t bytes, off_t offset, struct fuse_file_info * ffi){
	return -1;
}
...
struct fuse_operations myFS_operations = {
	...
	.unlink		= my_unlink,					// borra esa mierdaaaaaa
	.read		= my_read,						// read that shit
};
